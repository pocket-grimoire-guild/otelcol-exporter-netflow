...
its complete environment settings and requirement for a published immutable
tag. The recipe
...
matrix cells. Missing required values, invalid addresses, family or protocol
mismatches, unsupported required fields and width or timestamp failures reject
that record. Values are range checked; they are not clamped or silently truncated; the general IPFIX profile explicitly floors times to milliseconds.
Unknown and unselected attributes do not become wire loss and do not make an
otherwise encodable record invalid.

The recommended v9 timed and IPFIX general profiles require deployment attestation that original
source templates carried measured flow times and the receiver preserved them.
Check the prominent comment in the complete example before deployment. The
exporter re-encodes `flow.start`/`flow.end`; it never infers measured provenance
or substitutes receipt/export/envelope timestamps. Keep original-source identity
separate from this exporter's configured observation domain and UDP identity.
