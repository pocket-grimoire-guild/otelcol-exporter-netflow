...
// NewDataPacket returns a reusable incremental NetFlow v9 data appender.
...
func (p *dataPacketAppender) Append(record wire.WireRecord) error {
	...
	sysUptime, _ := elapsedMilliseconds(p.header.ExportTimeUnixNanos, p.header.UptimeOriginUnixNanos)
	if err := validateRecordTime(p.shape, record, p.header.UptimeOriginUnixNanos, sysUptime); err != nil {
		return err
	}
	...
	writeDataRecord(&c, p.shape, record, p.header.UptimeOriginUnixNanos)
